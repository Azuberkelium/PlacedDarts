document.addEventListener('DOMContentLoaded', () => {
    ...
    const createButton = (value, text, className, radius, angle) => {
    ...
    const buildDartboard = () => {
        ...
        dartNumbers.forEach((number, index) => {
            const angleInDegrees = (index * degreesPerSector) - 90 + (degreesPerSector / 2);
            const angleInRadians = (angleInDegrees * Math.PI) / 180;

            // Outer Number
            const numberRadius = 190;
            const numberButton = createButton(number, number, 'outer-number', numberRadius, angleInRadians);
            dartboardContainer.appendChild(numberButton);

            // Double Ring
            const doubleValue = number * 2;
            const doubleRadius = 170;
            const doubleButton = createButton(doubleValue, `D${number}`, 'double-ring', doubleRadius, angleInRadians);
            dartboardContainer.appendChild(doubleButton);

            // Triple Ring
            const tripleValue = number * 3;
            const tripleRadius = 120;
            const tripleButton = createButton(tripleValue, `T${number}`, 'triple-ring', tripleRadius, angleInRadians);
